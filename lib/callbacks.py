...
from server import page, cheesecake, user_state, state
...
def fabric_callback_handler(event):
    ...
    if sender_id in cheesecake.keys():
        user_state[sender_id] = state[3]
        image_id = cheesecake[sender_id].get('1st_image_id')
        ...
        db.session.commit()
        cheesecake[sender_id]['fabric_id'] = fabric.fabric_id
...
def pattern_callback_handler(event):
    ...
    if sender_id in cheesecake.keys():
        ...
        db.session.commit()
        cheesecake[sender_id]['pattern_id'] = pattern.pattern_id
        yes_no = [QuickReply(title="Yes", payload="YES"), QuickReply(title="No", payload="NO")]
